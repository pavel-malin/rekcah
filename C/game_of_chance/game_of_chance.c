...
// Custom user struct to store infomation about users
struct user 
{
	int uid;
	int credits;
...
int main()
{
	...
	srand(time(0)); // Seed randomizer with the current time.
	
	if (get_play_data() == -1)  // Try to read player data from file.
		register_new_player(); // If there is no data, registre a new player.
	while (choice != 7)
	{
		...
		printf("7 - Quit\n");
		printf("[Name: %s]\n", player.name);
		printf("[You have %u credist] -> ", player.credits);
		scanf("%d", &choice);

		if ((choice < 1) || (choice > 7))
			printf("\n[!!] The number %d is an invalid selection.\n\n", choice);
		else if (choice < 4)  // Otherwise, choice was a game of some scort.
		{
			if (choice != last_game) // If the function ptr isn't set
			{		 // the point it at the selected game
				if (choice == 1) 
...
void register_new_player()
{
	...
	printf("\nWelcome toTHe Game of Chance %s.\n",player.name);
...
// This function is usid to input the player name, since
...
void input_name()
{
	...
	while (input_char != '\n')  // Loop until newline.
	{
		*name_ptr = input_char; // Put the input char into name field.
		...
		name_ptr++; // Increment the pointer.
	}
...
// and the card the user has picjed as input. If the user_pick is
...
// This function input wages for both the No Match Dealer and 
// Find the Ace games. It expects the available credits and the
// previous wager as arguments. The preivous_wager is only important
// for the second wager in the Find the Ace game. The function
// returns -1 if the wager is too big or to little, and it returns
// the wager amount therwise.
int take_wager(int available_credits, int previos_wager)
{
	...
	{
		printf("Nice try you must wager a positive number!\n");
		return -1;
	}
	total_wager = previos_wager + wager;
...
// This function is the Pick Number game.
...
int pick_a_number()
{
	...
	printf("will win the jakpot of 100 credits!\n\n");
...
int dealer_no_match()
{
	...
	printf("\n::::::: No Match Dealer :::::::\n");
	printf("In this game, you can wager up to all off your credits.\n");
	...
	printf("\t\t::: Deling out 16 random numbers :::\n");
	for (i=0; i < 16; i++)
	{
...
int find_the_ace()
{
	...
	if (player.credits == 0)
	{
		printf("You don't have have any credits to wager!\n\n");
		return -1;
	}
	...
	{
		printf("Would you like to:\n[c]hange your pick\top\t[i]ncrease your wager?\n");
